...
class TopPlayers: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if passedData.year == Int(currentYear){
            if passedData.sec == 0 && passedData.row == 0 {
                ...
                return currentDatabase.playerList(rookie: true).count
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if passedData.year == Int(currentYear) {
            currentDatabase.getTop(yyyy: Int(currentYear)!, passed: passedData, label: label)
