...
class Main: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        let leaguesPerYear = leaguesPerYearFunc()
        let sortedLeaguesPerYear = leaguesPerYear.sorted(by: { $0.0 < $1.0 })
        var sum = 0
        for (_,v) in sortedLeaguesPerYear {
            sum+=v
        }
        if sum == 0 {
            objectsArray = [[String(currentYear)]]
        }
