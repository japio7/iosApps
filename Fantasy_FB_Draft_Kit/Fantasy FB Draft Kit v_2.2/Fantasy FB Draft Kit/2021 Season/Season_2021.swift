...
class Season_2021: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.section == 2 {
            performSegue(withIdentifier: "DepthCharts_2021", sender: self)
        }
        else if indexPath.section == 3 {
            performSegue(withIdentifier: "MyLeagues_2021", sender: self)
        }
            ...
            alert.addAction(UIAlertAction(title: "Yes", style: .destructive, handler: { (YesAction) in
                let okayAlert = UIAlertController(title: "Reset Draft", message: "The 2021 Draft has been reset!", preferredStyle: .alert)
            ...
            performSegue(withIdentifier: "TopPlayers_2021", sender: self)
        }
        table.deselectRow(at: indexPath, animated: true)
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "TopPlayers_2021" {
            if let destVC = segue.destination as? TopPlayers_2021 {
    ...
    func addLeagueSegue() {
        performSegue(withIdentifier: "AddLeague_2021", sender: UIAlertAction.self)
    }
    @IBAction func btnMyTeam(_ sender: UIButton) {
        performSegue(withIdentifier: "MyTeam_2021", sender: self)
    }
    
    @IBAction func btnDraftOrder(_ sender: UIButton) {
        performSegue(withIdentifier: "DraftOrder_2021", sender: self)
    }
    @IBAction func btnAddLeague(_ sender: UIBarButtonItem) {
        if Database_2021.shared.roster_array() == [] {
