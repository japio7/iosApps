...
class TeamDepthChart: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        if passedData.year == currentYear {
            currentDatabase.loadTeamData(sec: passedData.sec!, row: passedData.row!, view1: view1, view2: view2, label: label, table: table)
        }
    }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if passedData.year == currentYear {
            return currentDatabase.data.count
        }
        return 0
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var playerData : [Cell] = []
        if passedData.year == currentYear {
            playerData = currentDatabase.data
        }
