...
class AddUnlistedPlayer: UIViewController {
    ...
    func setPlayerData() {
        playerName = txtPlayerName.text!.trimmingCharacters(in: .whitespaces)
        teamName = txtTeamName.text!.trimmingCharacters(in: .whitespaces)
        position = txtPosition.text!.trimmingCharacters(in: .whitespaces)
    }
    
    func playerData_isValid() -> Bool {
        if playerName != "" && currentDatabase.isValidTeam(tn: teamName.lowercased()) && currentDatabase.isValidPos(pos: position.lowercased()) {
            return true
        }
        return false
    }
    ...
    @IBAction func btnToTeam(_ sender: BorderedButton) {
        ...
        if playerName != "" && playerData_isValid() {
            ...
            btnToTeam.borderWidth = 1
            btnToTeam.borderColor = .blue
            team = currentDatabase.convertTeam(tn: teamName, label: lblMessage).0
            bye = currentDatabase.convertTeam(tn: teamName, label: lblMessage).1
    ...
    @IBAction func add(_ sender: UIButton) {
        if btnToTeam_isEnabled {
            ...
            currentDatabase.allPlayersAppend(playerData: player_data)
            currentDatabase.addToRosterList(player: player_data)
            currentDatabase.addToDraftList(player: player_data)
